...
class KKDB{
	...
	private function __construct($host, $user, $pwd=null, $db=null){
		$this->m_conn = mysql_connect($host, $user, $pwd, $db);
		$db && 	mysql_select_db($db, $this->m_conn);
	}
	
	public static function forge($host, $user, $pwd=null, $db=null){
		if(null == self::$m_instance){
			self::$m_instance = new self($host, $user, $pwd, $db);
		}
